...
class Keyring_Service_Strava extends Keyring_Service_OAuth2 {
	...
	function basic_ui_intro() {
		echo '<p>' . sprintf( __( 'You\'ll need to <a href="%s">register a new application</a> on Strava so that you can connect.', 'keyring' ), 'https://www.strava.com/settings/api' ) . '</p>';
	...
	function build_token_meta( $token ) {
		...
		if ( Keyring_Util::is_error( $response ) ) {
			...
			return apply_filters( 'keyring_access_token_meta', $meta, 'strava', $token, $profile, $this );
		}
		return array();
	}
